        ...
        <View className="px-6 pt-4">
          <Card className="mb-4 border border-primary/20 bg-primary/5">
            <Text className="text-lg font-semibold text-text mb-1">
              {role === 'doctor' ? 'Patient report workspace' : 'My report center'}
            </Text>
            <Text className="text-sm text-text-secondary leading-5">
              {role === 'doctor'
                ? 'Upload shared documents for the selected patient and generate structured system reports in separate sections.'
                : 'Upload reports for your doctor and review uploaded documents separately from system-generated summaries.'}
            </Text>
          </Card>

          {role === 'doctor' ? (
            <Card className="mb-4 border border-border">
              <Text className="text-base font-semibold text-text mb-3">Select patient</Text>
              {patients.length === 0 ? (
            ...
            <View className="mb-4 flex-row flex-wrap">
              {categoryOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    <Text className={`text-sm ${selected ? 'text-primary' : 'text-text'}`}>
                      {option.label}
                    </Text>
                  </TouchableOpacity>
                );
          ...
          <Text className="text-xl font-bold text-text mb-3">Uploaded Reports</Text>
          {(overview?.uploaded_reports.length ?? 0) === 0 ? (
            <Card className="mb-4">
              <Text className="text-sm text-text-secondary">
                No uploaded reports available yet.
              </Text>
            </Card>
          ) : (
            overview?.uploaded_reports.map((report) => (
              <TouchableOpacity
                key={report.id}
          ...
          <Text className="text-xl font-bold text-text mb-3">System Generated Reports</Text>
          {(overview?.generated_reports.length ?? 0) === 0 ? (
            ...
            overview?.generated_reports.map((report) => (
              <TouchableOpacity
                key={report.id}
                ...
                <Card className="mb-3 border border-border">
                  ...
                  <View className="mt-3 flex-row items-center justify-between">
                    ...
                    {report.attachment_url ? (
                      ...
                      >
                        <Text className="text-xs font-semibold text-primary">Open PDF</Text>
                      </TouchableOpacity>
                    ) : null}
                  </View>
                </Card>
